...
###################################
###################################
def multiKrum(parameters: Dict[str, Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
    """
    multi krum passed parameters.
    ...
    sorted_distance = dict(sorted(distances.items(), key=lambda item: item[1]))
    normalized_distances = normalize_tensor_dict(sorted_distance)
    ...
    return new_params, normalized_distances


def normalize_tensor_dict(tensor_dict):
    # Extract tensor values
    values = torch.tensor([v.item() for v in tensor_dict.values()])

    # Get the min and max values
    min_value = torch.min(values)
    max_value = torch.max(values)

    # Normalize each tensor value
    normalized_dict = {k: (v - min_value) / (max_value - min_value) for k, v in tensor_dict.items()}
...
if __name__ == "__main__":
    ...
    params = {'1': {'1': torch.Tensor([[1, 2, 3], [4, 5, 6]]), '2': torch.Tensor([[7, 8, 9], [10, 11, 12]])},
              ...
              '3': {'1': torch.Tensor([[25, 26, 27], [28, 29, 30]]), '2': torch.Tensor([[31, 32, 33], [34, 35, 36]])},
                ...
                '10': {'1': torch.Tensor([[109, 110, 111], [112, 113, 114]]), '2': torch.Tensor([[115, 116, 117], [118, 119, 120]])}}
    sizes = {'1': 2, '2': 2, '3': 3}
    new_params, normalized_distances = multiKrum(params)
    print(new_params, "\n")
    print(normalized_distances, "\n")
