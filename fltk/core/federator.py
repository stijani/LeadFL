...
class Federator(Node):
    ...
    ###########################
    # estimated distance of each client from all other clients (normalised sum)
    normalized_client_distances = {}
    ###########################
    ...
    def create_clients(self):
        ...
        if self.config.single_machine:
            ...
            for client_id in range(1, self.config.world_size):
                ...
                #################################
                # self.normalized_client_distances.append(client.normalized_client_distance)
                # self.normalized_client_distances[client_id] = client.normalized_client_distance
                ################################
                if mal:
    ...
    def run(self):
        ...
        for communication_round in range(self.config.rounds):
            ...
            # ############
            # for k, v in self.normalized_client_distances.items():
    ...
    def exec_round(self, com_round_id: int):
        ...
        if self.config.defense_controller:
            if self.defense_controller_counter > 0:
                ...
                start_defense = False
        # #######################################
        # client training happen here - sequentially
        # #######################################
        for client in selected_clients:  # local client training
        ...
        updated_model, normalized_client_distances = self.aggregation_method(client_weights)  # ####
        # if self.config.aggregation != 'trmean':
        #     self.aggregation_method(client_weights, client_sizes, self.config.tm_beta)
        # elif self.config.aggregation != 'multikrum':
        #     updated_model, normalized_client_distances = self.aggregation_method(client_weights, client_sizes)
        ...
        self.logger.info(
            f'[Round {com_round_id:>3}] Federator has a Malicious accuracy of {mal_accuracy} and \
                loss={mal_loss}, malicious confidence={mal_confidence}')
        end_time = time.time()
        duration = end_time - start_time
    ...
    def init_wandb(self):
        ...
        wandb.init(
            ...
            name=run_name,)  # #########  name=run_name, entity="tudlab")
