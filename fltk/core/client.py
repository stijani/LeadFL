...
class Client(Node):
    ...
    def __init__(self, identifier: str, rank: int, world_size: int, config: Config, mal: bool = False,
        ...
        self.normalized_client_distance = normalized_client_distance
                            ...
                            for name, param in self.net.named_parameters():
                                if 'weight' in name:
                                    ...
                                    regularization = torch.norm(
                                        param-old_params[name].detach().to(self.device) - old_gradient_mine[name].detach().to(self.device) - ones * self.config.lr)  # noqa
                                    loss = loss + regularization if loss else regularization
                            #################################################################
                            # loss = self.config.regular_weight * self.regular_schedule * loss
                            loss = self.normalized_client_distance * self.regular_schedule * loss
                            #################################################################
